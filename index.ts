...
const onLogin = async ({ actorId }: SessionEvent): Promise<void> => {
  ...
  const sock = sockets.find(s => s.cid === actorId);
  if (!sock) return;

  const unread = (await getMyMail(actorId, "inbox")).filter(m => !m.read);
  if (unread.length > 0) {
    send([sock.id], `%ch%cyYou have ${unread.length} unread mail message${unread.length === 1 ? "" : "s"}.%cn`);
  }

  if (player.data?.tempMail) {
    send([sock.id], "%chMAIL:%cn You have an unsent draft. Use '@mail/proof' to review or '@mail/abort' to discard.");
  }
};
...
export const plugin: IPlugin = {
  ...
  init: () => {
    ...
    _expirySweepTimer = setInterval(() => {
      runExpirySweep().catch(e => console.error("[mail] expiry sweep error:", e));
    }, EXPIRY_SWEEP_MS);
    // Run an initial sweep on startup
    runExpirySweep().catch(e => console.error("[mail] startup sweep error:", e));
